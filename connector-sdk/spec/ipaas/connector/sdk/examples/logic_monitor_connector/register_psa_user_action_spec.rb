...
describe 'Logic Monitor Register LM User Action', :action do
  let(:action_template_id) { '0199a9a7-1270-7861-9a65-832e8c806c2e' }
  ...
  context 'run' do
    ...
    context 'with runbook' do
      ...
      it 'includes endpoint information' do
        ...
        expect(output[:user_name]).to eq(user_name)
        expect(encryptor.decrypt(output[:password]).length).to eq(SecureRandom.uuid.length)
        expect(output[:ticketing_endpoint_url]).to eq('https://example.com/inbound/1/abc/def')
      end
    end
  end
end
